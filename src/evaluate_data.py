...
def perform_pca_and_save_results(data, output_dir):
    ...
    # Plot and save the cumulative explained variance ratio
    plt.figure(figsize=(8, 5))
    plt.bar(range(1, len(cumulative_explained_variance) + 1), cumulative_explained_variance, alpha=0.6)
    plt.xlabel('Principal Components')
    plt.ylabel('Cumulative Explained Variance Ratio')
    plt.title('Cumulative Explained Variance Ratio by Principal Component')
    plt.grid(True)
    plot_path = os.path.join(output_dir, 'cumulative_explained_variance.png')
    plt.savefig(plot_path)
    plt.close()
    print(f"Cumulative Explained Variance Ratio plot saved to {plot_path}")
