...
csv_file_path = os.path.join(data_dir, 'processed_cleaned_data.csv')
...
phenotypes = df['Subject Diagnosis'].replace(['marasmic kwashiorkor', 'marasmus kwashiorkor'], 'kwashiorkor')
...
df_ranked = pd.concat([df[['ID', 'Subject Diagnosis']], df_ranked], axis=1)
ranked_principal_components = perform_pca_and_save_results(df_ranked.drop(columns=['ID', 'Subject Diagnosis']), 'ranked', ranked_dir)
...
df_normalized = pd.concat([df[['ID', 'Subject Diagnosis']], df_normalized], axis=1)
normalized_principal_components = perform_pca_and_save_results(df_normalized.drop(columns=['ID', 'Subject Diagnosis']), 'normalized', normalized_dir)
...
df_ranked = pd.concat([df_ranked.drop(columns=['Subject Diagnosis']), phenotypes], axis=1)
df_normalized = pd.concat([df_normalized.drop(columns=['Subject Diagnosis']), phenotypes], axis=1)
