...
def perform_pca_and_save_results(data, output_suffix, output_dir):
    ...
    principal_components = pca.fit_transform(data)  # Perform PCA on numerical data only
...
def perform_manova_and_save_results(principal_components, phenotypes, output_suffix, output_dir):
    ...
    df_pca['Phenotype'] = phenotypes  # Add phenotypes to the PCA results dataframe
...
def plot_3d_pca(principal_components, phenotypes, output_suffix, output_dir):
    # Encode phenotypes to numerical values
    ...
    # Extract numerical labels from the legend
...
df_ranked = df.iloc[:, 3:].rank()
df_ranked['ID'] = df['ID']
df_ranked['Phenotype'] = phenotypes
...
for column in numeric_columns:
    ...
    if p <= 0.03:  # Data is not normal
        failed_shapiro_count += 1
        df_normalized[column] = np.log1p(df_normalized[column] - df_normalized[column].min())  # Log transform

# Run Shapiro-Wilk test again after log transformation
...
# Write Shapiro-Wilk test results to a file
...
df_normalized['Phenotype'] = phenotypes
