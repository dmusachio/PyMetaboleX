...
def read_qc_config(config_file):
    """
    Read the QC configuration from a file.
...
def main():
    ...
    None
    """
    # Read QC configuration from file
    config = read_qc_config('src/config.txt')

    # File paths
    qc_file_path = 'input/qc.xlsx'
    cleaned_data_file_path = os.getenv('PROCESSED_DATA_FILE', 'processed/cleaned_data.csv')

    # Initialize and run QC pipeline
    qc_pipeline = MetaboliteQC(
        filter_column_constant=config['filter_column_constant'],
        filter_column_missing_rate_threshold=config['filter_column_missing_rate_threshold'],
        filter_row_missing_rate_threshold=config['filter_row_missing_rate_threshold'],
        filter_column_zero_rate_threshold=config['filter_column_zero_rate_threshold'],
        replace_outlier_method=config['replace_outlier_method'],
        nSD=config['nSD'],
        impute_method=config['impute_method'],
        verbose=config['verbose']
    )

    processed_data = qc_pipeline.run_QC_pipeline(cleaned_data_file_path)
    ...
    # RSD analysis
    rsd_threshold = config['rsd_threshold']
